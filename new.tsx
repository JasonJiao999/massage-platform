...
import { FaTwitter, FaInstagram, FaFacebook, FaLine, FaTiktok, FaComments } from 'react-icons/fa';
import { getOrCreateChatRoom } from '@/lib/actions';

// 【新增】导入 react-tweet 组件（注意：它是一个客户端中使用的包）
import { Tweet, TweetSkeleton } from 'react-tweet'; 


// --- 类型定义 (保持不变) ---
// ... (所有接口定义 WorkerDetailProps, Profile, Service, Shop, etc.)
interface Profile {
  id: string; nickname: string | null; avatar_url: string | null; bio: string | null; address_detail: string | null; tags: string[] | null; years: number | null; photo_urls: string[] | null; video_urls: string[] | null; feature: string[] | null; social_links: { [key: string]: string } | null; gender: string | null; nationality: string | null; qr_url: string[] | null;
}
interface Service { id: string; name: string | null; description: string | null; price: number | null; duration_value: number | null; duration_unit: 'minutes' | 'hours' | 'days' | null; }
interface Shop { id: string; name: string | null; address: string | null; slug: string | null; }
interface TimeSlot { start: string; end: string; }
interface Availability { [key: string]: TimeSlot[]; }
interface Booking { start_time: string; end_time: string; }
export interface WorkerDetailProps {
  worker: Profile; services: Service[]; shop: Shop | null; initialAvailability: Availability; existingBookings: Booking[]; fullAddress: string; reviews: any[]; 
}
...
const WorkerDetailClient: FC<WorkerDetailProps> = ({ worker, services, shop, initialAvailability, existingBookings, fullAddress, reviews }) => {
  ...
  useEffect(() => {
    setSelectedHour(null); setSelectedMinute(null); setSelectedTime(null);
  }, [selectedDate]);
  useEffect(() => {
    setSelectedMinute(null); setSelectedTime(null);
  }, [selectedHour]);
  useEffect(() => {
  ...
  const leftPanelStyle = { width: windowWidth >= 1024 ? '35%' : '100%' };
  const rightPanelStyle = { width: windowWidth >= 1024 ? '65%' : '100%' };
    ...
    <div className="container max-w-[1200px] py-[20px]">
      <div style={layoutStyle}>
        ...
        <div style={rightPanelStyle} className="w-full">
          {/* ... (Photo Carousel remains the same) ... */}
